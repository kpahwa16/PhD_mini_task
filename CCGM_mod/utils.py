...
def matrix_poly(matrix, d):
  x = torch.eye(d, device=device) + torch.div(matrix.to(device), d).to(device)
...
def get_parse_args():
  ...
  parser.add_argument('--every_degree',
                      '-N',
                      type=int,
                      default=10,
...
def read_label(root, idl):
  with open(root, 'r') as f:
    ...
    reader = np.array([
        np.array(list(map(int, x[10:].strip().split(" ")))) for x in reader[2:]
    ])
    reader = reader[:, idl]
...
def get_partitions(every_n_degree):
  ...
  partitions_anchors = np.linspace(0,
                                   partitions_anchors_num * every_n_degree,
                                   num=partitions_anchors_num + 1,
                                   endpoint=True,
                                   dtype=int)
